 ...
 *
*/
#ifndef _GAZEBO_RENDERING_GPUSONAR_HH_
#define _GAZEBO_RENDERING_GPUSONAR_HH_
...
//#include "gazebo/rendering/GpuSonarDataIterator.hh"
#include "GpuSonarDataIterator.hh"
...
namespace gazebo
{
  ...
  namespace rendering
  {
    ...
    /// \class GpuSonar GpuSonar.hh rendering/rendering.hh
    /// \brief GPU based sonar distance sensor
    class GZ_RENDERING_VISIBLE GpuSonar
      : public Camera, public Ogre::RenderObjectListener
    {
      ...
      public: GpuSonar(const std::string &_namePrefix,
          ScenePtr _scene, const bool _autoRender = true);

      /// \brief Destructor
      public: virtual ~GpuSonar();
      ...
      /// \brief Create the texture which is used to render sonar data.
      /// \param[in] _textureName Name of the new texture.
      public: void CreateSonarTexture(const std::string &_textureName);

      // Documentation inherited
      public: virtual void PostRender();

      /// \brief All things needed to get back z buffer for sonar data.
      /// \return Array of sonar data.
      /// \deprecated use SonarDataBegin() and SonarDataEnd() instead
      public: const float *SonarData() const GAZEBO_DEPRECATED(9.0);

      /// \brief Constant iterator to access sonar data
      public: typedef GpuSonarDataIterator<GpuSonar> DataIter;

      /// \brief Return an iterator to the begining of the sonar data
      public: DataIter SonarDataBegin() const;

      /// \brief Return an iterator to one past the end of the sonar data
      public: DataIter SonarDataEnd() const;

      /// \brief Connect to a sonar frame signal
      ...
      private: std::unique_ptr<GpuSonarPrivate> dataPtr;
    };
    /// \}
  }
}

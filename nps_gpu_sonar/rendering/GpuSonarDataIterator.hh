 ...
 *
*/
#ifndef GAZEBO_RENDERING_GPUSONARDATAITERATOR_HH_
#define GAZEBO_RENDERING_GPUSONARDATAITERATOR_HH_
...
namespace gazebo
{
  namespace rendering
  {
    ...
    /// \brief const Bidirectional iterator for sonar data
    ///
    /// This class contains the information needed to access Sonar Data
    ...
    template <typename F>
    class GpuSonarDataIterator
    {
      public: friend F;

      /// \brief Destructor
      public: ~GpuSonarDataIterator();

      /// \brief Operator ==
      /// \param[in] _rvalue The iterator on the right of the ==
      /// \return true iff the iterators point to the same reading
      public: bool operator==(const GpuSonarDataIterator &_rvalue) const;
      ...
      /// \return A struct of sonar data
      public: const GpuSonarData operator*() const;

      /// \brief Dereference operator iter->
      /// \return a shared pointer object at the iterator's index
      public: const std::unique_ptr<const GpuSonarData> operator->() const;

      /// \brief Advance iterator to next reading (prefix: ++it)
      /// \return reference to this pointer after advancing
      public: GpuSonarDataIterator<F> &operator++();
      ...
      public: GpuSonarDataIterator<F> operator++(int _dummy);
      ...
      protected: GpuSonarDataIterator(const unsigned int _index,
...
#include "GpuSonarDataIteratorImpl.hh"
